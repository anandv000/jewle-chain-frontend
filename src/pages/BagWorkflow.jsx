...
.il{font-weight:700;white-space:nowrap;font-size:11px}.iv{font-size:11px}
...
.instr-cell{padding:5px 8px;font-size:11px}.instr-cell:first-child{border-right:1px solid #000}
...
.wc:last-child{border-right:0}.wv{font-size:15px;font-weight:900}
...
<div class="top-header">
  <div class="top-left"><div class="info-grid">
    ...
    <span class="il">Order Date :</span><span class="iv">${fmt2(order.orderDate)}</span>
    <span class="il">Order No :</span><span class="iv"><strong>ORD-${order.bagId||"—"}</strong></span>
    <span class="il">Delivery Date :</span><span class="iv">${fmt2(order.deliveryDate)}</span>
...
<table><thead><tr><th style="width:70px">Acces.</th><th>Issue</th><th>Issue</th><th>Issue</th><th>Rec</th><th>Rec</th><th>Rec</th></tr></thead>
<tbody>${acceRows}</tbody></table>
<div class="sec">Department Workflow</div>
<table><thead><tr><th style="width:52px">Dept</th><th>Date</th><th>Worker</th><th>Issue</th><th>Rec</th><th>Diff</th><th>Dust</th></tr></thead>
<tbody>${deptRows}</tbody></table>
<div class="sec">Diamond Details</div>
<table><thead>
  <tr><th rowspan="2">Diamond Code</th><th rowspan="2">Size (mm)</th><th rowspan="2">Wt/pc</th><th rowspan="2">Pcs</th><th rowspan="2">Total WT</th><th colspan="2">Issue</th><th colspan="2">Return</th></tr>
      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:theme.surface, border:`1px solid ${theme.borderGold}`, borderRadius:16, width:"94vw", maxWidth:820, maxHeight:"92vh", display:"flex", flexDirection:"column", overflow:"hidden" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"20px 28px", borderBottom:`1px solid ${theme.borderGold}`, flexShrink:0 }}>
          <div>
            ...
            <div style={{ fontSize:12, color:theme.textMuted, marginTop:3 }}>Fill manual fields → Generate PDF → Print / Save as PDF in browser</div>
          </div>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"24px 28px" }}>
                ...
                <div>
                  ...
                  <div style={{ background:theme.surfaceAlt, border:`1px solid ${theme.borderGold}`, borderRadius:8, overflow:"hidden" }}>
                    <table style={{ width:"100%", borderCollapse:"collapse" }}>
                      <thead><tr style={{ borderBottom:`1px solid ${theme.borderGold}` }}>{["Shape","Size","Wt/pc","Pcs","Total WT"].map(h=><TH key={h}>{h}</TH>)}</tr></thead>
                      <tbody>{order.diamondShapes.map((d,i)=>(
          ...
          {tab === "dept" && (
            <div>
              <div style={{ fontSize:12, color:theme.textMuted, marginBottom:16 }}>Gram history from workflow is pre-filled in Issue/Rec. Add worker names, dates, diff, and dust.</div>
              <div style={{ overflowX:"auto" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"16px 28px", borderTop:`1px solid ${theme.borderGold}`, flexShrink:0 }}>
          <div style={{ fontSize:12, color:theme.textMuted }}>💡 New tab will open — use browser Print → Save as PDF</div>
...
// ── Quick Gram Modal (for mark done from list card) ────────────────────────────
const QuickGramModal = ({ order, onClose, onConfirm }) => {
  ...
  const used = gramRemaining !== "" && !isNaN(parseFloat(gramRemaining))
    ? (currG - parseFloat(gramRemaining)).toFixed(2) : null;
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", backdropFilter:"blur(3px)", zIndex:300, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:theme.surface, border:`1px solid ${theme.borderGold}`, borderRadius:14, padding:28, width:"92vw", maxWidth:420 }}>
        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:19, color:theme.gold, marginBottom:6 }}>
          ✦ Step {order.currentStep + 1}: {STEPS[order.currentStep]}
        </div>
        ...
        <div style={{ background:theme.surfaceAlt, border:`1px solid ${theme.borderGold}`, borderRadius:10, padding:14, marginBottom:18, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontSize:10, color:theme.textMuted, marginBottom:4 }}>GOLD BEFORE THIS STEP</div>
            <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:28, color:theme.textMuted }}>{currG}g</div>
          </div>
          <div style={{ fontSize:12, color:theme.textMuted, textAlign:"right" }}>Weigh gold now,<br/>enter remaining</div>
        </div>
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:11, color:theme.textMuted, textTransform:"uppercase", marginBottom:6 }}>Remaining Gold After Step (grams)</div>
          <input
            type="number" value={gramRemaining} placeholder={`max: ${currG}`} autoFocus
            onChange={e=>{setGramRemaining(e.target.value);setGramError("");}}
        ...
        <div style={{ display:"flex", gap:12 }}>
          <button onClick={confirm} disabled={saving} className="btn-primary" style={{ flex:1, padding:12, fontSize:14 }}>
...
const BagWorkflow = ({ orders, setOrders, customers = [] }) => {
  ...
  const [quickMarkOrder, setQuickMarkOrder] = useState(null); // for card-level mark done
  ...
  if (selectedId && order) {
    ...
    const canMark     = order.currentStep < STEPS.length && !isCompleted;
  ...
  return (
    <div className="fade-in">
              ...
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:14 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:8, flexShrink:0 }}>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    {/* Mark Done — on card, for any non-completed bag */}
                    {canMark && (
                      <button
                        onClick={e=>{e.stopPropagation(); setQuickMarkOrder(o);}}
                        style={{ display:"inline-flex", alignItems:"center", gap:6, background:`${theme.success}18`, border:`1px solid ${theme.success}50`, color:theme.success, padding:"5px 12px", borderRadius:7, fontFamily:"'DM Sans'", fontSize:12, cursor:"pointer", transition:"all 0.2s" }}
                    ...
                    {/* Bag Sheet PDF — ALL bags */}
                    <button
                      onClick={e=>{e.stopPropagation(); setPdfOrder(o);}}
                      style={{ display:"inline-flex", alignItems:"center", gap:6, background:`${theme.gold}15`, border:`1px solid ${theme.gold}50`, color:theme.gold, padding:"5px 12px", borderRadius:7, fontFamily:"'DM Sans'", fontSize:12, cursor:"pointer", transition:"all 0.2s" }}
      ...
      {/* Quick Mark Done Modal (from card) */}
      {quickMarkOrder && (
        <QuickGramModal order={quickMarkOrder} onClose={()=>setQuickMarkOrder(null)} onConfirm={handleQuickConfirm}/>
      )}
