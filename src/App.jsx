...
import { customerAPI, folderAPI, orderAPI } from "./services/api";
...
const NAV = [
  ...
  { id: "diamonds",     path: "/diamonds",       label: "Diamond Shapes", icon: "diamond"   },
...
const AppLayout = ({ user, onLogout }) => {
  ...
  // Load all base data once on login
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError("Cannot connect to backend. Make sure your server is running on port 5000.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div style={{ display:"flex", minHeight:"100vh", background:theme.bg }}>
      ...
      <div style={{ flex:1, padding:"36px 40px", overflowY:"auto" }}>
        <Routes>
          ...
          <Route path="/products"      element={<Products     folders={folders}     setFolders={setFolders} />} />
          <Route path="/diamonds"      element={<DiamondShapes />} />
          <Route path="/create-order"  element={<CreateOrder  customers={customers} folders={folders} orders={orders} setOrders={setOrders} />} />
